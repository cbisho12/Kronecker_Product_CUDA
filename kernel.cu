...
void kronk_kernel(int* A_Col,int* B_Col,VType* A_Val,VType* B_Val,int* C_Col,VType* C_Val, int na, int counter, int size_kk, int start_k, int start_kk ) {  
  ...
  //** Global Mem is slower than local Mem, so local variables are created so Global Var only accessed once

 int s_size_kk = size_kk;
 int s_start_kk = start_kk;
 int s_start_k = start_k;
 int s_counter = counter;
 int s_na = na;
